...
from coinmarketcapapi import CoinMarketCapAPI, CoinMarketCapAPIError  # New import
...
@tool()
def get_coinmarketcap_price_history(
    token_symbol: str, candle_interval: CandleInterval, num_candles: int
) -> Dict[str, Any]:
    """
    Retrieves historical price data for a token using CoinMarketCap API.
    Falls back to Binance if CoinMarketCap data can't be retrieved.
    """
    # Min value of 2 ensures we have at least two data points for calculating trends
    num_candles = min(max(2, int(num_candles)), 1000)
    
    try:
        # Initialize CoinMarketCap API client with sandbox mode and API key
        api_key = "b54bcf4d-1bca-4e8e-9a24-22ff2c3d462c"
        cmc = CoinMarketCapAPI(api_key=api_key, sandbox=True)
        
        # Convert interval from our format to CoinMarketCap format
        # Since weekly is supported, we can map directly
        interval_mapping = {
            "1d": "daily",
            "1h": "hourly", 
            "1w": "weekly"  # Weekly is supported!
        }
        cmc_interval = interval_mapping.get(candle_interval, "daily")
        
        print(f"Using interval: {cmc_interval} for {token_symbol}")
        
        try:
            # Try to get cryptocurrency quotes with symbol parameter
            print(f"Requesting historical data for {token_symbol} with {cmc_interval} interval")
            historical_data = cmc.cryptocurrency_ohlcv_historical(
                symbol=token_symbol,
                time_period=cmc_interval,
                count=num_candles
            )
            
            # Check if we got a valid response with data
            if historical_data.data and token_symbol.upper() in historical_data.data:
                token_data = historical_data.data[token_symbol.upper()]
                
                if "quotes" in token_data and token_data["quotes"]:
                    quotes = token_data["quotes"]
                    print(f"Successfully retrieved {len(quotes)} {cmc_interval} quotes for {token_symbol}")
                    
                    # Format data to match Binance return format
                    klines = []
                    for quote in quotes:
                        # Process timestamp - sandbox format may be different
                        if "time_open" in quote:
                            timestamp = quote["time_open"]
                        elif "timestamp" in quote:
                            timestamp = quote["timestamp"]
                        else:
                            # If timestamp format changes, try to extract it from available fields
                            for key in quote.keys():
                                if "time" in key.lower() or "date" in key.lower():
                                    timestamp = quote[key]
                                    break
                            else:
                                # If no timestamp field found, use current time as fallback
                                timestamp = time.strftime("%Y-%m-%dT%H:%M:%S.000Z", time.gmtime())
                        
                        ohlc = quote["quote"]["USD"]
                        ...
                        try:
                            ...
                            if isinstance(timestamp, str):
                                ...
                                try:
                                    if "." in timestamp:  # Has milliseconds
                                        ts = int(time.mktime(time.strptime(timestamp, "%Y-%m-%dT%H:%M:%S.%fZ")) * 1000)
                                    else:  # No milliseconds
                                        ts = int(time.mktime(time.strptime(timestamp, "%Y-%m-%dT%H:%M:%SZ")) * 1000)
                                except ValueError:
                                    # Try other common formats if standard format fails
                                    try:
                                        ts = int(time.mktime(time.strptime(timestamp, "%Y-%m-%d")) * 1000)
                                    except ValueError:
                                        # Last resort - use current time
                                        ts = int(time.time() * 1000)
                            else:
                                # If timestamp is already a number, assume it's seconds since epoch
                    ...
                    if klines:
                        return {
                            ...
                            "source": "coinmarketcap_sandbox"
                        }
                    else:
                        print("No valid candles could be processed from the response")
                else:
                    print(f"No quotes found in response for {token_symbol}")
            else:
                print(f"Token {token_symbol} not found in response")
                
        except Exception as e:
            print(f"Error fetching from CoinMarketCap: {e}")
            
        # If we get here, CoinMarketCap failed, so fall back to Binance
        return _get_binance_fallback(token_symbol, candle_interval, num_candles)
            
    except Exception as e:
        print(f"Unexpected error in get_coinmarketcap_price_history: {e}")
        return _get_binance_fallback(token_symbol, candle_interval, num_candles)

def _get_binance_fallback(token_symbol, candle_interval, num_candles):
    """Helper function to fall back to Binance data source"""
    print(f"Falling back to Binance for {token_symbol}")
    binance_data = get_binance_price_history.invoke(
        {
            "token_symbol": token_symbol,
            "candle_interval": candle_interval,
            "num_candles": num_candles,
    ...
    if "error" not in binance_data:
        binance_data["source"] = "binance_fallback"
    return binance_data
...
@tool()
def analyze_price_trend_cmc(
    token_symbol: str, candle_interval: str, num_candles: int
) -> Dict[str, Any]:
    """
    Analyzes price trend for a token using CoinMarketCap data including moving averages, volatility metrics,
    and enhanced technical indicators over the specified time period.
    """
    try:
        # Get the price history first using CoinMarketCap
        price_data = get_coinmarketcap_price_history.invoke(
            {
                "token_symbol": token_symbol,
                "candle_interval": candle_interval,
                "num_candles": num_candles,
        ...
        return {
            "error": f"Error analyzing price trend for {token_symbol} using CoinMarketCap: {e}",
        }
...
@tool()
def compare_assets_cmc(
    token_symbols: List[str], candle_interval: CandleInterval, num_candles: int
) -> Dict[str, Any]:
    """
    Compare performance of multiple crypto assets using CoinMarketCap data with simplified insights for average investors.
    ...
    for token_symbol in token_symbols:
        try:
            # Use the CoinMarketCap version for analysis
            analysis = analyze_price_trend_cmc.invoke(
                {
                    "token_symbol": token_symbol,
                    "candle_interval": candle_interval,
                    "num_candles": num_candles,
...
@tool()
def max_drawdown_for_token_cmc(
    token_symbol: str,
    candle_interval: CandleInterval = CandleInterval.DAY,
    num_candles: int = 90,
) -> Dict[str, Any]:
    """
    Calculates the maximum drawdown for a cryptocurrency using CoinMarketCap price data
...
@tool()
def analyze_volatility_trend_cmc(
    token_symbol: str,
    candle_interval: CandleInterval = CandleInterval.DAY,
    num_candles: int = 90,
) -> Dict[str, Any]:
    """
    Analyzes the trend in volatility for a cryptocurrency over the specified time period using CoinMarketCap data.
    """
    try:
        # Get price data from CoinMarketCap
        price_data = get_coinmarketcap_price_history.invoke(
